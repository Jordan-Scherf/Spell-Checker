...
Program Description: This Program is essentially a spellchecker program that takes in
 a dictionary to be placed into a hashmap then reads in another file to spell check it then offer
...
int main() {
    ...
    std::cout << '\n';
    std::string word;
    // Taking in each word and capatilizing them then placing them into the hashmap
    while(file >> word){
        for (auto& c : word) {
            c = std::toupper(c);
        }
...
*/
    ...
    for( auto it = misspelled.begin(); it != misspelled.end(); it++) {
        ...
        for (int i = Wordcheck.length()-1; i >= 0; i--) {
            ...
            for (char j = 'A'; j != 'Z' + 1; j++) {
                ...
                if(dictionary.find(Wordcheck) != dictionary.end()){
    ...
    std::cout << "time taken = " << (clock() - tStart);//CLOCKS_PER_SEC;
    return 0;
}
